...
- **Look elegido (jul 2026, validado por captura):** desaturación 50% hacia un gris **sesgado a verde enfermizo ("luna mala")** — no gris neutro ni azul frío. Fórmula en `src/phantom.c` `Phantom_TintPaletteRange`: gris de luminancia, luego tono `tr=gray*12/16, tg=gray*17/16 (clamp 31), tb=gray*10/16`, mezclado 50% con el original. Verificado en el sandbox: menús/UI NO se desaturan (el tinte no toca el `LoadPalette` global). El special `PhantomReloadOverworldPalettes` (`LoadMapTilesetPalettes(gMapHeader.mapLayout)`) fuerza que el efecto se vea al instante tras el `setflag`, sin necesitar un warp.
